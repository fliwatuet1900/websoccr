...
echo FormBuilder::createFormGroup($i18n, 'playerid', array('type' => 'foreign_key', 'jointable' => 'spieler',
		'entity' => 'player', 'labelcolumns' => 'vorname,nachname,kunstname'), '', 'match_manage_createplayer_label_');
...
foreach ($teamPrefixes as $teamPrefix) {
	...
	$playerTable = $website->getConfig('db_prefix') . '_spiel_berechnung AS SB';
	...
	if (!$playersCount) {
		...
		$fresult->free();
		if ($formationCount && $formationCount['hits']) echo '<p><a href="?site=$site&match=$matchId&team=$teamPrefix&action=generate" class="btn"><i class="icon-hand-right"></i> '. $i18n->getMessage('match_manage_playerstatistics_createfromfrmation') . '</a></p>';
